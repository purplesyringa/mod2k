...
// that `y' = ax + by`. At the end of the algorithm, `y' = 1`, so we infer `ax = 1 (mod y)`, i.e.
// `a = x^-1 (mod y)`. By applying the same operations to `(u, v) = (1, 0)` in parallel, we obtain
// `u' = au + bv = a`, i.e. the inverse of `x`.
//
// The linear operations we perform are subtraction and division by powers of two. For `x`, the
// divisions are exact, but they're not guaranteed to be such for `u`, so we have to use a trick:
// - If modular arithmetic is cheap, we can work with `u, v` modulo `m`, since we only need
//   `u mod m`. This utilizes right shifts provided by the modular type.
    ...
    (prime = $prime:literal, strategy = builtin) => {
        fn inverse(self) -> Option<Self> {
            ...
            while x != 0 {
                ...
                // SAFETY: At any point, `current x <= max(initial x, initial y)`, thus
